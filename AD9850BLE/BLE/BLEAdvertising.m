...
- (BOOL)scanStart
{
	...
	// 探索対象のデバイスが持つサービスを指定
	NSArray *services = [NSArray arrayWithObjects:[CBUUID UUIDWithString:BLE_ADVDATA_SERVICE_UUID], nil];
				  ...
				  RSSI:(NSNumber *)RSSI
{
	...
	if ([serviceUUIDs containsObject:[CBUUID UUIDWithString:BLE_ADVDATA_SERVICE_UUID]]) {
		// 対象とするサービスUUIDが含まれている
		//if ([peripheral.name compare:BLE_MY_SERVICE_NAME] == NSOrderedSame) {
		if ([localName compare:BLE_ADVDATA_SERVICE_NAME] == NSOrderedSame) {
			// 対象とするサービス名である
